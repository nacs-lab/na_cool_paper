...
\begin{figure*}
  ...
  \caption{(A) Energy levels and schematic of Raman sideband cooling.
    ...
    \fxnote{TALK ABOUT THIS IN THE TEXT}.
...
and quantum simulation of many-body systems \fxnote{REF}.
This approach provides full single-site resolution and control, and can feature long-range interactions via Rydberg atoms, polar molecules, or dipolar atoms \fxnote{REF}.
...
Optical tweezers can also be re-arranged in real-time, allowing rapid preparation of atoms in complex geometries with high fidelity \fxnote{REF}.
...
This has been demonstrated for neutral Rb\cite{Thompson2013,Kaufman2012} and Cs \fxnote{REF} in experiments that had low initial temperature and small Lamb-Dicke parameter.
These features
may not be easily achievable for other systems, such as directly laser-cooled polar molecules \fxnote{REF},
...
$\{\omega_1,\omega_2,\omega_3\}/2\pi = \{69, 430, 590\}\ \text{\text{kHz}}$ \fxnote{ERROR BARS}
which corresponds to optical pumping Lamb-Dicke parameters of
$\{\eta^{OP}_1,\eta^{OP}_2,\eta^{OP}_3\} = \{0.60, 0.24, 0.21\}$ \fxnote{ERROR BARS}.
...
do not reach their minimums at the same time \fxnote{FIG}, using multiple orders of motional sidebands
...
the anharmonicity of the trap away from the harmonic center. This results in sidebands that are broader and of reduced height \fxnote{FIG}, and limits the Rabi frequency of the Raman pulse
on the radial sidebands to be no lower than tens of kHz in order to drive atoms in
different motional states equally. \fxnote{EXPLAIN THIS A LITTLE MORE}.
...
we begin the cooling with $\{\bar n_2, \bar n_3\}=2.2, 1.9$ \fxnote{ERROR BARS}.
...
and yields $90\%$ and $92\%$ \fxnote{ERROR BARS},
...
Improvements on these \fxnote{HOW?} should improve the cooling performance even further.
