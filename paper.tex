...
Systems of single neutral atoms trapped in optical tweezers are an exciting platform to study
...
and can feature long-range interactions via Rydberg atoms, polar molecules,
...
the entropy.
The second step could also change the motional state of the atoms which could cause heating.
...
is the Lamb-Dicke parameter for optical pumping.
Additional photon scattering on the atom in the $|F=2, m_F=-2\rangle$
state will also cause heating.
In order to reduce this effect, we address the atom in the $F=2$ states during optical pumping
on the D1 line, so that the $|F=2, m_F=-2\rangle$ state is a dark state for $\sigma^-$
polarization and does not scatter any photon. We measured a $130(20)$ times reduction in photon
scattering rate on $|F=2, m_F=-2\rangle$ state compare to using only the D2 line
with the same optical pumping rate.
...
\cite{Hutzler2017-LightShifts}, but leave on the OP light with constant intensity.
...
The simulation also shows that the most efficient way of cooling on a high order sideband is
to perform a $\pi$ pulse corresponding to its maximum matrix element.
...
This strategy is only not as effective during the last stage of cooling in each direction
where most of the remainly excited population is in the first motional excited state
while the rest also have the strongest coupling to the first cooling sideband.
Because of this, we switch to cooling only on the first order sideband as the last cooling
stage while maintaining the alternating order between different axis to maximize the final
ground state probability.
Guided by the simulation result, we construct our initial cooling sequence by first starting
