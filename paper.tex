...
\begin{abstract}
  ...
  We demonstrate that Raman sideband cooling to the 3D motional ground state is possible %outside the Lamb-Dicke regime,
...
Each experimental cycle ends with a second imaging sequence that measures the hyperfine state of the atom as $|F=2\rangle$ or $|F=1\rangle$~\footnote{The final hyperfine state sensitive detection employs a strong beam that is resonant with the $F=2$ to $F'=3$ cycling transition, to heat the $F=2$ population out of the trap.  Therefore, any atoms which survive this heat out procedure are interpreted as having been in the $F=1$ state.}.%a repetition of the experiment that was initially bright and remained bright after the pulse sequence is interpreted as a final F=1 atomic state.}.
...
The weak axial direction %, $(\eta^{\textrm{OP}}_{z,\textrm{eff}})^2>1$
is far outside of the LD regime ($\eta^2\ll 1$), and the probability of recoil heating per OP step is large. %~\footnote{For example, the average change in the motional states for the initial distribution is $2.4$ in the axial direction, which is significantly greater than $0.89-0.95$in previous experiments of RSC of Rb and Cs~\cite{Li2012,Kaufman2012,Thompson2013,Liu2017}.},
